...
package CBIL::Util::Hify;

=pod

=head1 Name

C<CSP::Hify> - an object that makes creating HTML easy.  Should
probably be retired in favor of C<CGI.pm>
...
sub AUTOLOAD {
	 ...
	 # a element method, e.g, e_p, e_P, or P
	 ...
	 if ($name =~ /^E_([a-zA-Z]+)$/i ||
       $name =~ /^([A-Z][A-Z0-9]*)$/
			...
			return $M->_unwrapper({ tag => $element,
                              arg => \@_,
                            });
	 }
...
sub _unwrapper {
	 ...
	 # not a reference, just a string: ready to go with a wrapper
	 if (not ref $arg) {
			return $M->_dispatcher({ arg => { viz => $arg, },
                               tag => $A->{tag},
                             })
   }

	 # list of something or other: unwrap further
	 elsif ( ref $arg eq 'ARRAY' ) {
			return join('',
                  map {
                     $M->_unwrapper({ tag => $A->{tag},
                                      arg => $_,
                                    })
                  } @$arg
								 );
	 ...
	 # ref of a ref; de-reference and try again.
	 elsif ( ref $arg eq 'REF' ) {
			return $M->_unwrapper({ arg => $$arg,
                              tag => $A->{tag},
                            });
	 }

	 # a plain hash: ready to as is.
...
sub es {
	 ...
	 foreach my $k_tagType ( 'atts', 'depa' ) {

			# add all attributes of current type
			foreach my $metatag ( @{ $A->{ $k_tagType } } ) {

				 #print STDERR "CSP::Tohtml: metatag=$metatag ";
...
sub ep {
	 ...
	 if (defined $intro and $outro) {
			...
			return undef unless defined $A->{ vals }->{ 'viz' };
			$middle = $A->{ vals }->{ viz };
...
= (
	 ...
	 del     => {
							 p => 1,
							 a => [
										 @HIFY::a_CoreAttrs, $HIFY::a_cite, $HIFY::a_datetime
										],
							},
	 ...
	 img      => {
								...
								a => [
											@HIFY::a_CoreAttrs, @HIFY::a_i18n, @HIFY::a_Events,
											$HIFY::a_src, $HIFY::a_alt, $HIFY::a_longdesc, $HIFY::a_height,
											$HIFY::a_width, $HIFY::a_usemap, $HIFY::a_ismap,
										 ],
							 },
...
sub __test {
   ...
   my $hify = new CBIL::Util::Hify;


	 #my $hify = new ooTESS::CSP::Hify;
	 ...
	 print  $hify->e_textarea({ viz  => "That's what I'm talkin' 'bout!",
                              rows => 5,
                              cols => 70,
                            });
