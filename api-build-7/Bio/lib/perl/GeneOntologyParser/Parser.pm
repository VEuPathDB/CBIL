...
sub loadFile {
  ...
  while (<$fh>) {
	...
	if (/!(autogenerated-by.+)$/){
	    $store->setDescription("Gene Ontology release of branch $branch, version $version, derived from file $fileName from GO site.  File notes: " . $1); 
	}
	if (/!date:\s+(.+)/){
	    $store->setReleaseDate($1);
	}
    }
     else {
