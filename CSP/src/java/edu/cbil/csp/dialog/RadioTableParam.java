 ...
 * RadioTable.java
 *
 * An enumerated value parameter specifically for a small number of enumerated values. It will create an HTML
 * table with three columns (radio button, value , description) The user may choose only <b>one</b> of the
 * enumerated options. The options are all represented by Strings.
 * <p>
 *
 ...
 */
public class RadioTableParam extends StringParam {
   ...
   *          {@link cbil.csp.dialog.Dialog}).
   * @param descr
   *          A short description of the element.
   * @param help
   *          A help string describing the element"s usage.
   * @param st
   *          {@link cbil.csp.StringTemplate} that controls the appearance of the element itself.
   * @param ht
   *          {@link cbil.csp.StringTemplate} that controls the appearance of the element"s help text.
   * @param prompt
   ...
   * @param defs
   *          The definitions for the enumeration values presented to the user.
   * @param initial
   *          Initial value. Must be an element of <code>values</code>
   * @param optional
   *          Whether the user is allowed to omit the parameter.
   */
  protected RadioTableParam(String name, String descr, String help, StringTemplate st, StringTemplate ht,
      String prompt, String values[], String labels[], String defs[], String initial, boolean optional,
    ...
    if (labels != null) {
      if (labels.length != n_values)
        throw new IllegalArgumentException(
            "RadioTableParam: number of labels does not " + "match number of values.");
    ...
    if (defs != null) {
      if (defs.length != n_values)
        throw new IllegalArgumentException(
            "RadioTableParam: number of definitions does not " + "match number of values.");
   ...
   * Constructor that accepts an initial value.
   */
  public RadioTableParam(String name, String descr, String help, StringTemplate st, StringTemplate ht,
      String prompt, String values[], String labels[], String defs[], String initial, boolean optional) {
   ...
   */
  public RadioTableParam(String name, String descr, String help, StringTemplate st, StringTemplate ht,
      String prompt, String values[], String labels[], String defs[]) {
  ...
  public Item copy(String url_subs) {
    RadioTableParam ep = new RadioTableParam(name, descr, help, template, help_template, prompt, values,
        labels, defs, initial_value, optional, value2label);
  ...
  public String[] getHTMLParams(String help_url) {
    ...
    for (int i = 0; i < n_values; i++) {
      ...
      result.append(
          HTMLUtil.TR(HTMLUtil.TD(new AH(new String[] { "bgcolor", "#CFCFCF" }), HTMLUtil.INPUT(base)) +
              HTMLUtil.TD(((labels == null) ? values[i] : labels[i]) + "&nbsp;&nbsp;") + ((defs == null) ? ""
  ...
  @Override
  public RadioTableParam copyParam(String new_name) {
    RadioTableParam ep = new RadioTableParam(new_name, descr, help, template, help_template, prompt, values,
        labels, defs, initial_value, optional, value2label);
    return ep;
  }
