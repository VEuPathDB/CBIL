 ...
 *
 * An {@link edu.cbil.csp.dialog.ItemGroup} that can be "expanded" (i.e. the {@link edu.cbil.csp.dialog.Item}s
 * within it are shown) or "collapsed" (i.e. the items within it are not shown) under user control. Such item
 * groups should typically be used only for sets of additional parameters that are either optional or that
 * have default values.
 * <p>
 *
...
public class ExpandableItemGroup extends ActionItemGroup implements ActionHandler {
  ...
  /**
   * HTML-specific. The URL to which the group's <code>Action</code>s will point. Whatever script or program
   * is lurking at this URL should know how to direct the action request back to this object's
   ...
   * @param my_url
   *          URL to which hide/show requests should be directed.
   * @param show_string
   *          Description of the show action for the user.
   * @param hide_string
   *          Description of the hide action for the user.
   * @param initial_state
   *          Whether the group is initially expanded.
   */
  public ExpandableItemGroup(String name, String descr, String help, StringTemplate st, StringTemplate ht,
      String bgcolor, boolean initial_state, String my_url, String show_string, String hide_string) {
    ...
    show = new Action(show_name, "Show", "help", null, null, "submit", show_string, my_url);
    hide = new Action(hide_name, "Hide", "help", null, null, "submit", hide_string, my_url);
  }
  ...
  /**
   * Handle an action that was generated by either <code>hide</code> or <code>show</code>. Changes the current
   * state of the item group, which must then be redisplayed.
   */
  @Override
  ...
  public Item copy(String url_subs) {
    ExpandableItemGroup result = new ExpandableItemGroup(name, descr, help, template, help_template, bgcolor,
        expanded, my_url, show_string, hide_string);
  ...
  public Item getItemByName(String itemName, boolean recurse) {
    System.out.println(
        "ExpandableItemGroup: called with " + itemName + " have show = " + show_name + "and hide = " + hide_name);

    if (itemName.equals(show_name)) {
      return show;
    }
    else if (itemName.equals(hide_name)) {
      return hide;
    }
    return super.getItemByName(itemName, recurse);
  }
  ...
  public String[] getHTMLParams(String help_url) {
    ...
    if (expanded) {
      return new String[] { super_params[0] + "&nbsp;&nbsp;&nbsp;&nbsp;" + hide.makeHTML(help_url),
          super_params[1], super_params[2] };
    }
    else {
      return new String[] { super_params[0] + "&nbsp;&nbsp;&nbsp;&nbsp;" + show.makeHTML(help_url),
          super_params[1], "" };
    }
  }
