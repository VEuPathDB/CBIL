 ...
 * TreeEnumParam.java
 *
 * A hierarchical enumerated value/controlled vocabulary. Capable of generating either HTML or JavaScript.
 *
 * TO DO:
 *
 * Generate only 1 copy of the JavaScript makeSelect function in the case where there are multiple
 * TreeEnumParams on one page (the form will still work correctly, but there will be multiple redundant copies
 ...
 */
public class TreeEnumParam extends StringParam {
  ...
  public TreeEnumParam(String name, String descr, String help, StringTemplate st, StringTemplate ht,
      String prompt, boolean optional, int sizeHint, TreeNode vocab) {
  ...
  public Item copy(String url_subs) {
    return new TreeEnumParam(name, descr, help, template, help_template, prompt, optional, sizeHint, vocab);
  }
  ...
  public String[] getHTMLParams(String helpURL) {
    ...
    String select = (makeJavaScript(treeVar) + "\n<BR>\n" + "<FONT SIZE=\"-1\">\n" +
        ...
        "ONCHANGE=\"makeSelect('1', '" + formName + "', '" + name + "', '-1', " + treeVar + ");\">" +
        makeOptionList() + "</SELECT>\n" + "<SCRIPT LANGUAGE=\"JavaScript1.1\">\n" + "<!--\n" + // begin HTML
                                                                                                // comment
        "makeSelect('-1', '" + formName + "', '" + name + "', '-1', " + treeVar + ");\n" + "// -->\n" + // end
                                                                                                        // HTML
                                                                                                        // comment
        "</SCRIPT>");
  ...
  @Override
  public TreeEnumParam copyParam(String newName) {
    return new TreeEnumParam(newName, descr, help, template, help_template, prompt, optional, sizeHint, vocab);
  }
  ...
  protected void appendJavaScriptData(TreeNode t, String treeVar, StringBuffer sb, boolean isRoot,
      String parentID) {
    TreeNode kids[] = t.kids;
    int nKids = kids.length;
  ...
  /**
   * Generate JavaScript that implements the multilevel selection capability. The JavaScript is escaped to
   * prevent it being interpreted by non-compliant browsers.
   */
  ...
  /**
   * Generate a plain HTML list of <OPTION>s (for reverse compatability with non-JavaScript browsers.)
   */
