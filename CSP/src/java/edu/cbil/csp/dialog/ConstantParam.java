 ...
 * ConstantParam.java
 *
 * A subclass of {@link edu.cbil.csp.dialog.Param} that represents a parameter whose value is constant. Yes,
 ...
 */
public class ConstantParam extends StringParam {
   ...
   * @param prompt
   *          Prompt string used to goad the user into entering data. In this case the user cannot enter data,
   *          so the prompt serves instead as a description of the value (assuming that the parameter is not
   *          hidden.)
   * @param hidden
   ...
   */
  public ConstantParam(String name, String descr, String help, StringTemplate st, StringTemplate ht,
  ...
  public Item copy(String url_subs) {
    return new ConstantParam(name, descr, help, template, help_template, prompt, hidden, initial_value);
  }
  ...
  public String[] getHTMLParams(String help_url) {
    String input_elt = HTMLUtil.INPUT(
        new AH(new String[] { "type", "hidden", "name", name, "value", initial_value }));

    if (hidden)
      return new String[] { input_elt, "", "" };
    return new String[] { input_elt + HTMLUtil.B(prompt), HTMLUtil.I(initial_value), "" };
  }
  ...
  public StringTemplate getDefaultTemplate() {
    if (hidden) {
      ...
      return new StringTemplate(HTMLUtil.TR(HTMLUtil.TD(left2, ps[0] + "&nbsp;")), ps);
    }
