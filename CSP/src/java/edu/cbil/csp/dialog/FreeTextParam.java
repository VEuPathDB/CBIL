 ...
 *
 */
public class FreeTextParam extends StringParam {

  protected int width_hint, height_hint;
  protected StringValidatorI validator;

  public FreeTextParam(String name, String descr, String help, StringTemplate st, StringTemplate ht,
      String prompt, boolean optional, int width_hint, int height_hint, String initial,
      String[] samples, StringValidatorI validator) {
    super(name, descr, help, st, ht, prompt, optional);
    this.initial_value = initial;
    this.sample_values = samples;
  ...
  public FreeTextParam(String name, String descr, String help, StringTemplate st, StringTemplate ht,
      String prompt, boolean optional, int width_hint, int height_hint, String initial_value,
      String sample_values[]) {
    this(name, descr, help, st, ht, prompt, optional, width_hint, height_hint, initial_value, sample_values,
        null);
  }
  ...
  public Item copy(String url_subs) {
    return new FreeTextParam(name, descr, help, template, help_template, prompt, optional, width_hint,
        height_hint, initial_value, sample_values, validator);
  }
  ...
  public StringTemplate getDefaultTemplate() {
    ...
    return new StringTemplate(
        HTMLUtil.TR(HTMLUtil.TD() + HTMLUtil.TD(left, ps[0]) + HTMLUtil.TD(right, ps[2])) +
            HTMLUtil.TR(HTMLUtil.TD() + HTMLUtil.TD(textarea, ps[1])),
        ps);
  }
  ...
  public String[] getHTMLParams(String help_url) {
    ...
    return new String[] { anchor + prompt,
        HTMLUtil.TEXTAREA(new AH(new String[] { "name", this.name, "rows", Integer.toString(height_hint),
            "cols", Integer.toString(width_hint) }), (initial_value == null) ? "" : initial_value)
        // + HTMLUtil.BR() +
  ...
  @Override
  public FreeTextParam copyParam(String new_name) {
    return new FreeTextParam(new_name, descr, help, template, help_template, prompt, optional, width_hint,
        height_hint, initial_value, sample_values, validator);
  }
