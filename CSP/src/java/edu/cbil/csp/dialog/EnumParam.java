 ...
 * EnumParam.java
 *
 * An enumerated value parameter.  The user may choose only <b>one</b>
...
public class EnumParam extends Param {
	...
	if (labels != null) {
		...
		throw new IllegalArgumentException("EnumParam: number of labels does not " +
    ...
    public EnumParam(String name, String descr, String help,
		     StringTemplate st, StringTemplate ht, String prompt,
		     String values[], String labels[], String initial, 
	...
	if ((n_values <= TALL) && (labels == null)) {
	    ...
	    String select = HTMLUtil.SELECT
		(new AH(new String[] {"name", name,
				      "size", size}),
		 result.toString());
