 ...
 * MultiParam.java
 *
 * An enumerated value parameter. The user may choose more than <b>one</b> of the enumerated options. The
 * options are all represented by Strings.
 * <p>
 *
...
public class MultiParam extends EnumParam {
   ...
   */
  public MultiParam(String name, String descr, String help, StringTemplate st, StringTemplate ht,
      String prompt, String values[], String labels[], boolean optional) {
   ...
   *          The names for the enumeration values presented to the user.
   * @param value2label
   *          Hashtable mapping of values to a label.
   */

  public MultiParam(String name, String descr, String help, StringTemplate st, StringTemplate ht,
      String prompt, String values[], String labels[], Hashtable<String,String> value2label) {
    super(name, descr, help, st, ht, prompt, values, labels, null, false, 5, value2label);

  }

  public MultiParam(String name, String descr, String help, StringTemplate st, StringTemplate ht,
      String prompt, String values[], String labels[], boolean optional, Hashtable<String,String> value2label) {
  ...
  public Item copy(String url_subs) {
    MultiParam mp = new MultiParam(name, descr, help, template, help_template, prompt, values, labels,
        value2label);
    return mp;
  }
  ...
  public void storeHTMLServletInput(HttpServletRequest rq) {
    ...
    if (input_value != null)
      this.current_values = input_value;
  }
    ...
    for (int j = 0; j < input.length; j++) {
      ...
      if (!success) {
        errors.append("The value selected, '" + input[j] + "', is not among " +
            ...
            "and click on the link next to the parameter to " + "see more detailed help information. ");
        return false;
      }
    }
    return true;
  ...
  public StringTemplate getDefaultTemplate() {
    ...
    return new StringTemplate(
        HTMLUtil.TR(HTMLUtil.TD(new AH(new String[] { "align", "right", "valign", "middle" }), ps[0]) + "\n" +
            HTMLUtil.TD(left, "&nbsp;&nbsp;" + ps[1]) + "\n" +
            HTMLUtil.TD(new AH(new String[] { "align", "right", "valign", "top" }),
  ...
  public String[] getHTMLParams(String help_url) {
    ...
    for (int i = 0; i < n_values; i++) {
      AH base = new AH(new String[] { "type", "checkbox", "name", name, "value", values[i] });
