 ...
 *
 * An {@link edu.cbil.csp.dialog.ItemGroup} that also contains one or more actions. These actions will
 * typically affect the group's items in some way (e.g. adding and removing them).
 * <p>
 *
...
public class ActionItemGroup extends ItemGroup {
   ...
   */
  public ActionItemGroup(String name, String descr, String help, StringTemplate st, StringTemplate ht,
      String bgcolor) {
   ...
   *
   * @param a
   *          The <code>Action</code> to remove.
   */
  ...
  /**
   * Return the <code>Action</code> at a given index; by default actions are indexed by the order in which
   * they were added.
   *
   * @param in
   *          The index of the <code>Action</code> to return.
  ...
  /**
   * Copy a set of actions from one <code>ActionItemGroup</code> to another. Used to support the
  ...
  public Item copy(String url_subs) {
    ActionItemGroup result = new ActionItemGroup(name, descr, help, template, help_template, bgcolor);
  ...
  public StringTemplate getDefaultTemplate() {
    ...
    return new StringTemplate(HTMLUtil.TR(HTMLUtil.TD(
        HTMLUtil.TABLE(new AH(new String[] { "cellpadding", "5", "width", "100%", "border", "0", }),
            HTMLUtil.TR(HTMLUtil.TD(new AH(new String[] { "bgcolor", bgcolor }),
                HTMLUtil.FONT(new AH(new String[] { "size", "+0" }), HTMLUtil.B(ps[0])) + "&nbsp;&nbsp;" +
                    ps[1]))) +
            HTMLUtil.TABLE(ps[2]))),
        ps);
  }
  ...
  public String[] getHTMLParams(String help_url) {
    ...
    return new String[] { super_params[0], action_str.toString(), super_params[1] };
  }
