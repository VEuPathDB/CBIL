 ...
 * PasswordParam.java
 *
 * Subclass of {@link edu.cbil.csp.dialog.Param} that accepts a password. It differs from <code>Param</code>
 * only insofar as it renders to an HTML element that does not echo its output to the screen.
 * <p>
 *
...
public class PasswordParam extends StringParam {
   ...
   * @param optional
   *          Whether the parameter is optional (usually not the case for passwords).
   */
  public PasswordParam(String name, String descr, String help, StringTemplate st, StringTemplate ht,
  ...
  public String[] getHTMLParams(String help_url) {
    return new String[] { makeHTMLAnchor(false) + prompt,
        HTMLUtil.INPUT(new AH(new String[] { "name", this.name, "value",
            ((current_value != null) ? current_value : ""), "type", "password", "size", "25" })),
