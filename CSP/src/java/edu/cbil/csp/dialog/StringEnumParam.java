 ...
 *
 * This is a hybrid of an enumerated parameter. The user may either Type in a response, or choose from one of
 * the enumerated values, Or choose an enumerated value and then modify it by typing. The options are all
 ...
 * @author Phillip Le
 */
public class StringEnumParam extends StringParam {
   ...
   */
  protected StringEnumParam(String name, String descr, String help, StringTemplate st, StringTemplate ht,
    ...
    if (labels != null) {
      if (labels.length != n_values)
        throw new IllegalArgumentException(
            "StringEnumParam: number of labels does not " + "match number of values.");
  ...
  public StringEnumParam(String name, String descr, String help, StringTemplate st, StringTemplate ht,
      String prompt, String values[], String labels[], String initial, boolean optional, int size_hint) {
  ...
  public Item copy(String url_subs) {
    StringEnumParam ep = new StringEnumParam(name, descr, help, template, help_template, prompt, values,
        labels, initial_value, optional, size_hint, value2label);
  ...
  public String[] getHTMLParams(String help_url) {
    ...
    if (initial_value != null) // Puts in the initial value if there is one.
      sb.append(initial_value);
    sb.append("\" SIZE=\"50\">\n" + "<INPUT TYPE=\"hidden\" NAME=\"" + name + "_hidden\" VALUE=\"\">\n" +
        "</FONT>\n" + "<BR CLEAR=\"both\">\n" +

        "<SELECT NAME=\"" + name + "_description\" SIZE=\"" + size + "\" " + "ONCHANGE=\"makeSelect" + name +
        "('1', '" + formName + "', '" + name + "_description', '" + name + "');\">" + makeOptionList() +
        "</SELECT>\n" + "<SCRIPT LANGUAGE=\"JavaScript1.1\">\n" + "<!--\n" + // begin HTML comment
        "makeSelect" + name + "('1', '" + formName + "', '" + name + "_description', '" + name + "');\n" +
        "// -->\n" + // end HTML comment
        "</SCRIPT>");
  ...
  protected String makeFunc(String selectName) {
    StringBuffer sb = new StringBuffer();

    sb.append("\n\n<SCRIPT LANGUAGE=\"JavaScript1.1\">\n");
    sb.append("<!--\n"); // begin HTML comment
    sb.append("\n");
    sb.append("function makeSelect" + selectName + "(id, formName, selName, modName) {\n");
  ...
  @Override
  public StringEnumParam copyParam(String new_name) {
    StringEnumParam ep = new StringEnumParam(new_name, descr, help, template, help_template, prompt, values,
        labels, initial_value, optional, size_hint, value2label);
    return ep;
  }
