 ...
 * {@link edu.cbil.csp.dialog.Dialog} is itself an <code>Itemgroup</code>.<P>
 *
...
public class ItemGroup extends Item {
     ...
     *                 element's help text.
     * @param bgcolor  Sugggested background color.
     */
    public ItemGroup(String name, String descr, String help, 
	...
	for (int i = 0;i < n_items;i++) {
		...
		all_ok = false;
		item_errors.append(HTMLUtil.TABLE
				   (HTMLUtil.TR
				    (HTMLUtil.TD(error.toString()))));
	    }
	}

	if (!all_ok) {
	    errors.append(HTMLUtil.TABLE
			  (HTMLUtil.TR
			   (HTMLUtil.TD
			    (HTMLUtil.B(this.descr))) +
			   HTMLUtil.TR
			   (HTMLUtil.TD
			    (item_errors.toString()))));
	}
	return all_ok;
    }
	...
	return new StringTemplate(HTMLUtil.TR
				  ...
				  HTMLUtil.TR
				  (HTMLUtil.TD
				   (HTMLUtil.TABLE(new AH(new String[] {"width", "100%",
									"border", "0",
									"cellpadding", "0",
									"cellspacing", "0"}), 
						   ps[1]))) + "\n", ps);
	...
	return new String[] {(descr == null) ? "" : this.descr,
				 itemtext.toString()};
    }
	...
	return new String[] {this.help, help.toString()};
    }
     ...
     *
     * @param from      Source item group.
     * @param to        Source item group.
     * @param url_subs  Perl-style substitution to apply to any URLs
     */
    ...
    /**
     * Retrieve an item at a particular index.  By default the 
     * items within an item group are indexed by the order that 
    ...
    /**
     * Add an item at a particular index.  Similar to 
     * {@link java.util.Vector#insertElementAt}.
     *
    ...
    /**
     * Add an item.  By default it will occupy the next 
     * available index.
     *
     * @param di     The item to add.
     * @return       A boolean indicating whether the operation succeeded.
    ...
    /**
     * Add an item to a specific item group (either this one or one
     * contained within it.)  Performs a recursive search if necessary.
     *
     * @param ig     The item group to add to.
     * @param di     The item to add.
     * @return       A boolean indicating whether the operation succeeded.
     */
     ...
     *
     * @param name        The name of the item(s) to remove.
     * @param recurse     Whether to recursively search for the item(s).
     * @param remove_all  Whether to remove all occurrences (<code>true</code>) or 
     *                    only the first (<code>false</code>).
     * @return The number of occurrences that were actually removed.
     */
    public int removeItemByName(String name, boolean recurse, boolean remove_all) {
	return removeItemByPattern("/^" + name + "$/", recurse, remove_all);
    }

    /**
     * Remove an item by name, using a regular expression.
     *
     * @param pattern     Items whose names match this regex will be removed.
     * @param recurse     Whether to recursively search for the item(s).
     * @param remove_all  Whether to remove all occurrences (<code>true</code>) or 
     *                    only the first (<code>false</code>).
     * @return The number of occurrences that were actually removed.
	...
	if (recurse) {
		...
		if (di instanceof ItemGroup) {
		    ...
		    if (!remove_all) return n_removed;
		}
	    }
	}
	return n_removed;
    ...
    /**
     * @param name      The name of the item to be replaced.
     * @param new_item  A replacement for the named item.
     * @param recurse   Whether to recursively search for the item.
     * @return          A boolean indicating whether the operation succeeded.
     */
     ...
     * @param recurse Whether to recursively search for the named item.
     * @return The ItemGroup that is the parent of the item named <code>name</code>,
     *         or <code>null</code> if that item cannot be found.
     */
    public ItemGroup getParentByName(String name, boolean recurse) {
	int n_items = items.size();
     ...
     *
     * @param pattern Regular expression used to select the items to be returned.
     * @return An array of {@link edu.cbil.csp.dialog.Item}s, each of which
    ...
    /**
     * Get <b>all</b> items of a specific (exact) class.  Does not return
     * instances of subclasses of the given class, only direct instances.
     *
     ...
     *
     * @param name     The name of the desired {@link edu.cbil.csp.dialog.Dialog}
