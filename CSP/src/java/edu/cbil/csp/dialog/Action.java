 ...
 * Action.java
 *
 * An action that a user can invoke from a {@link edu.cbil.csp.dialog.Dialog}. In the context of an HTML form,
 * it will typically be rendered as a "submit" input element, but if constructed with its own URL, it can also
 * be rendered as a plain hyperlink (with the obvious proviso that when a user clicks on the link, any data
 * entered into the form itself will be lost).
 * <p>
 *
...
public class Action extends Item implements ActionHandler {
  ...
  /**
   * HTML-specific; the URL to use if the <code>Action</code> is to be rendered as a hyperlink.
   */
   ...
   * @param type
   *          The type of input element, either "reset" or "submit" (HTML-specific)
   * @param value
   *          A value to be submitted with the HTML form when the Action is invoked.
   */
  public Action(String name, String descr, String help, StringTemplate st, StringTemplate ht, String type,
      String value) {
   ...
   *
   * An alternate constructor that accepts a URL; in an HTML context this allows the Action to be rendered as
   * a simple hyperlink rather than as a button.
   *
   * @param name
   *          Unique String used to identify the action in the context of a larger input structure (e.g. a
   *          {@link edu.cbil.csp.dialog.Dialog}).
   ...
   *          A value to be submitted with the HTML form when the Action is invoked.
   * @param url
   *          The URL to fetch when the Action is invoked.
   */
  public Action(String name, String descr, String help, StringTemplate st, StringTemplate ht, String type,
    ...
    if (!type.equals("reset") && !type.equals("submit"))
      throw new IllegalArgumentException(
          "Action: type must be 'reset' or 'submit'; '" + type + "' is not allowed.");
  }
  ...
  public String[] getHTMLParams(String help_url) {
    ...
    if ((url != null) && (!type.equals("reset")))
      return new String[] { HTMLUtil.A(new AH(
          new String[] { "href", url + "&" + HTMLUtil.urlEncode(name) + "=" + HTMLUtil.urlEncode(value) }),
          "[" + value + "]") };
    else {
      ...
      //
      return new String[] {
          (HTMLUtil.INPUT(new AH(new String[] { "type", type, "name", name, "value", value }))) };
    }
  }
